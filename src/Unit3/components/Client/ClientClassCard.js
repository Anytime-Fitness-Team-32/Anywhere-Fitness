...
export default function ClientClassCard(props) {
  ...
  const handleReserve = () => {
    setIsReserved(true);
    axiosWithAuth()
      .patch(`${baseURL}/classes/class/${c.classid}/addclient/`)
  ...
  const handleCancelReserve = () => {
    setIsReserved(false);
    axiosWithAuth()
      .patch(`${baseURL}/classes/class/${c.classid}/removeclient/`)
      .then((res) => console.log(res))
      .catch((err) => console.log(err));
  };
  ...
  return (
    <DivMain color={props.color}>
      <DivContent>
        ...
        {isReserved && (
          <button onClick={handleCancelReserve}>Cancel Reservation</button>
        )}
      </DivContent>
    </DivMain>
  );
